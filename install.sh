...
check_token() {
  ...
  if [ "$USER_TOKEN" = "jetzofficial" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy dulu Gih Ke JetzOfficial${NC}"
    echo -e "${YELLOW}TELEGRAM : @JetzOffc${NC}"
    echo -e "${YELLOW}WHATSAPP : 085722271832${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 10K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}©JetzOfficial${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer JetzOfficial Private   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${WHITE}ggg:     gg.     -   ,ggg       • WhatsApp : 0895405753446    ${NC}"
  echo -e "${WHITE} ggg:     ggg._    _,ggg        • Credit  : JetzOfficial ${NC}"
  echo -e "${WHITE} ggg.    '.'''ggggggp           • Support by JetzOfficial${NC}"
